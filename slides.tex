    ...
    \begin{column}{.45\textwidth}
      \begin{animateinline}[
        ...
        \multiframe{10}{rP = 0.1 +.1, r = 1 + 1}{ 
         ...
         \path (3,3) -- (1.25, 1.25) node[pos=\rP] (q){};
         \draw[post, color=red] (3,3) -- (q.west);
 ...
 \begin{block}{}
  ...
  %\begin{columns}[T] 
   ...
   \begin{bclogo}[logo=\bccrayon,arrondi=0.1]{} 
     A \textbf{matching} for a robot is a function $\eta :
     \{\emptyset,e_{ij}, e_{ik}, \ldots\} \rightarrow \{\id(r_a),
     \id(r_b), \ldots \} $ that associates each neighbor ID with
     either a lattice graph edge from its role vertex or with the
     null value $\emptyset$.
   \end{bclogo}   
   % \end{column}%
   % \begin{column}{.45\textwidth}
